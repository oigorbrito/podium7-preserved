...
class MultiSourceValidationTests(unittest.TestCase):
    def corpus(self):
        return (
            MultiSourceCase("corroborated-make", "vehicle:corroborated", "make", (fact("nhtsa_vpic", "corroborated", "make", "VOLVO"), fact("eea_co2_cars", "corroborated", "make", "VOLVO")), "CANONICAL", "two qualified source families agree on the normalized make"),
            MultiSourceCase("single-source-regulatory-variant", "vehicle:eea-only", "eea.variant", (fact("eea_co2_cars", "eea-only", "eea.variant", "UZH4"),), "CANONICAL", "EEA regulatory variant remains a source-scoped fact rather than retail trim"),
            MultiSourceCase("conflicting-power", "vehicle:power-conflict", "power", (fact("eea_co2_cars", "power-conflict", "power", 186, "kW"), fact("nhtsa_vpic", "power-conflict", "power", 190, "kW")), "CONFLICT", "normalized disagreement must remain an explicit conflict"),
            MultiSourceCase("registration-year-not-model-year", "vehicle:year-boundary", "eea.registration_year", (fact("eea_co2_cars", "year-boundary", "eea.registration_year", 2025),), "CANONICAL", "registration year is retained only in its source semantic namespace"),
            MultiSourceCase("unsupported-retail-trim", "vehicle:trim-gap", "variant", (), "REVIEW", "regulatory source fields do not justify a retail variant guess"),
            MultiSourceCase("unsupported-manufacture-year", "vehicle:manufacture-year-gap", "manufacture_year", (), "REVIEW", "model or registration year must not be substituted for manufacture year"),
        )

    def test_bounded_corpus_measures_corroboration_conflict_review_and_provenance(self):
        summary = evaluate_multisource_cases(self.corpus())["summary"]
    ...
    def test_non_conflict_results_have_no_conflict_state(self):
        report = evaluate_multisource_cases(self.corpus())
        self.assertTrue(all(item["conflictState"] is None for item in report["results"] if item["disposition"] != "CONFLICT"))
    ...
    def test_duplicate_candidate_fact_ids_fail_closed(self):
        ...
        with self.assertRaisesRegex(ValueError, "candidate fact ids must be unique"):
            MultiSourceCase("duplicate-fact", "vehicle:duplicate", "make", (candidate, SourceCandidate("eea_co2_cars", candidate.fact)), "CANONICAL", "same fact cannot count as two source candidates")

    def test_candidate_container_must_be_tuple(self):
        with self.assertRaisesRegex(ValueError, "candidates must be a tuple"):
            MultiSourceCase("wrong-container", "vehicle:test", "make", [], "REVIEW", "malformed fixture")  # type: ignore[arg-type]
    ...
    def test_required_text_and_source_id_cannot_be_whitespace(self):
        ...
        with self.assertRaisesRegex(ValueError, "case_id is required"):
            MultiSourceCase("   ", "vehicle:test", "make", (), "REVIEW", "missing evidence")
        with self.assertRaisesRegex(ValueError, "rationale is required"):
            MultiSourceCase("blank-rationale", "vehicle:test", "make", (), "REVIEW", "   ")
