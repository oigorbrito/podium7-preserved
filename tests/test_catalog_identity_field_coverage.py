...
class CatalogIdentityFieldCoverageTests(unittest.TestCase):
    ...
    def test_case_only_raw_variants_have_deterministic_order(self) -> None:
        ...
        self.assertEqual(["SUV", "suv"], report["values"])
        self.assertEqual({"suv": ["SUV", "suv"]}, report["normalizationCollisions"])

    def test_standalone_cli_bootstraps_repository_import_without_site_packages(self) -> None:
        env = dict(os.environ)
        ...
        result = subprocess.run(
            [
                sys.executable,
                "-S",
                str(ROOT / "scripts" / "measure_catalog_identity_coverage.py"),
                "--help",
            ],
            cwd=ROOT,
            env=env,
        ...
        self.assertIn("Measure consumer-visible catalog identity-field coverage", result.stdout)
