...
def _augmented_global_benchmark(path: Path) -> None:
    ...
    payload["sources"].append(
        {
            ...
            "title": "Ford inicia a venda do Mustang GT Performance de setima geracao no Brasil",
    ...
    targets[0]["sourceIds"].append(GT_GENERATION_SOURCE)
...
def _probe_records() -> list[dict]:
    overlay = {
        ...
        "datasetVersion": "probe-global-mustang-clean-1",
...
class MustangCleanProbeTests(unittest.TestCase):
    def test_probe_builds_all_four_composite_sides_with_partitioned_generation_sources(self) -> None:
