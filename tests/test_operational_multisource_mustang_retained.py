...
class MustangRetainedRolloutTests(unittest.TestCase):
    def test_composed_measurement_promotes_exact_four_mustang_sides(self) -> None:
        ...
        self.assertEqual(summary["replayableRecords"], 54)
        self.assertEqual(summary["blockedRecords"], 6)
        ...
        self.assertEqual(summary["replayableByMethod"][MULTISOURCE_REPLAY_METHOD], 32)
        self.assertEqual(
        ...
        self.assertEqual(tuple(_key(record) for record in records[-4:]), MUSTANG_KEYS)
        for record in records[-4:]:
            generation_sources = record["payload"]["provenance"]["fieldEvidence"]["generation"]
            self.assertNotIn(SPEC_SOURCE, generation_sources)

    def test_mustang_sides_replay_after_retained_corpus_with_complete_field_provenance(self) -> None:
        store = CatalogStore()
        ...
        mustang_records = records[-4:]
        mustang_results = results[-4:]
        allowed = {
            CatalogIngestionAction.CREATED,
            CatalogIngestionAction.MATCHED,
            CatalogIngestionAction.REVIEW,
        }
        self.assertTrue(all(result.action in allowed for result in mustang_results))
        self.assertGreaterEqual(len(store.catalog_vehicle_ids_page(after_id=None, limit=100)), 14)

        for record, result in zip(mustang_records, mustang_results):
