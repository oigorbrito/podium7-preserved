...
class CatalogIdentityBrazilBenchmarkTests(unittest.TestCase):
    def test_brazil_golden_slice_is_balanced_and_source_backed(self) -> None:
        dataset = load_catalog_identity_benchmark(DATASET)

        self.assertEqual(dataset.version, "br-1.0")
        self.assertEqual(len(dataset.source_ids), 10)
    ...
    def test_brazil_golden_slice_baseline_has_no_false_merge_or_overcommit(self) -> None:
        dataset = load_catalog_identity_benchmark(DATASET)
