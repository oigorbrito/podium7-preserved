...
3. its body must match its recorded acquisition hash; and
4. the exact body SHA-256 must equal the pinned expected digest.

Failure of any terms check produces `REVIEW_REQUIRED` with `mutation_required=false`. Terms drift is therefore an operational/compliance review state, not a source-data conflict and not an automatic legal conclusion.
...
- locator mismatch and acquisition-hash mismatch;
