...
- PR #106, `Exhaust current production reviews V2`, is open/draft. Latest hardening binds each human-review disposition to an exact `(caseId, side)` review item instead of a case-wide decision. The current 13-review topology is explicitly regression-gated; a missing side becomes unassessed and a stale side becomes unused rather than inheriting silently. Earlier head `8f776f35722d25e5f419209907068c22f54fc953` had executable green CI in run `32709518982`; latest head `cb478aa3f7b4baaeddc2131a511c69f01e81c22e` triggered run `32793306279`, where both lanes failed before steps (`steps=null`). The active plan is `docs/exec-plans/active/PRODUCTION-REVIEW-EXHAUSTION-V2.md`.
- PR #107, `Expose catalog review operator CLI V1`, is open/draft at `bd4bd3051e9fa4b44543e7537e84877a1aec6fe0`. The canonical `python -m podium7 review list|show|match|create` surface performs an existing-database read-only schema/version preflight, exposes source/canonical candidate context on `show`, and delegates mutations to the established audited domain functions. Self-review found that legacy `scripts/resolve_catalog_review.py` still bypassed that safety boundary and could initialize a database; it is now only a compatibility wrapper around the canonical CLI. `docs/CATALOG-REVIEW-OPERATOR-V1.md` records the contract on that branch. Run `32793587681` failed both lanes pre-step.
...
- PR #109, `Wire market-first infrastructure principle into workflow`, is open/draft at `a0c793b367d048dd34f73d0bfffdda230daf1d8a`. It links accepted ADR-0001 from `ARCHITECTURE-PRINCIPLES.md` and `DEVELOPMENT-WORKFLOW.md` without duplicating the policy. Its CI also fails pre-step.
