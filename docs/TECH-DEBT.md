...
Status: `OPEN / FROZEN ACQUISITION + CROSS-FAMILY EXTRACTION LOCALLY_VERIFIED`
...
Frozen acquisition lineage is now independently gated across the current two-family benchmark inventory. The manifest is derived from the existing benchmark case inventories and covers 16/16 retained snapshots across 13 exact HTTPS source URLs. Every snapshot is checked against a pinned repository blob identity and produces a verified SHA-256 content-addressed reference; the measured retained corpus has zero acquisition-evidence issues. Missing, empty, mutated, duplicated, unpinned/orphaned and non-HTTPS acquisition evidence fails explicitly.

This establishes repository-local deterministic extraction reuse across two different web source families plus reproducible integrity of the retained acquisition evidence. It still does **not** establish heterogeneous-web or production precision/recall, live acquisition/navigation completeness, source-discovery completeness, production source distribution, or a general MPGe semantic mapping.

The remaining debt is therefore **live acquisition/navigation/source-discovery generalization and broader production source distribution/corpus coverage**. Any expansion should add independently inspected source-family evidence and measured failure modes. Do not weaken strict validation, silently accept missing/semantically incompatible data, or select a generic browser/agent layer merely to increase nominal page coverage.
