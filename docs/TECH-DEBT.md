...
Status: `OPEN / FROZEN ACQUISITION + DIRECT HTTP TRANSPORT + RETAINED-URL LIVE SAMPLE + BROWSER REFUSAL CHARACTERIZED + CROSS-FAMILY EXTRACTION LOCALLY_VERIFIED`
...
The repository therefore has local deterministic evidence for retained-snapshot lineage, direct HTTP transport semantics, extraction reuse across two frozen web source families, one bounded real-network direct-HTTP compatibility observation, and one bounded browser characterization of the currently refused Autoevolution family. It still does **not** establish heterogeneous-web or production precision/recall, compatibility with arbitrary public sources, source-discovery completeness, robots/rate/politeness behavior, production source distribution, comprehensive SSRF/DNS-rebinding defense, or a general MPGe semantic mapping.

The remaining acquisition/generalization debt is therefore **finding compliant alternative live source paths where the current source family refuses both direct HTTP and a normal browser identity, source discovery, robots/rate/politeness policy, broader production source distribution/corpus coverage, and stronger SSRF/DNS-rebinding defenses before untrusted arbitrary locator acquisition**. Browser automation can be reconsidered only when a different independently measured source family demonstrates a genuine JavaScript requirement and successful normal-browser acquisition; do not add a generic browser/agent layer merely because direct HTTP failed.

Any expansion should add independently inspected source-family evidence and measured failure modes. Do not weaken strict validation, silently accept missing/semantically incompatible data, or select a generic crawler/browser/agent layer merely to increase nominal page coverage.
