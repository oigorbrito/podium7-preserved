...
`PRODUCTION-EVIDENCE-ENRICHMENT-V3.md` closes the next measured evidence cycle. The current Volkswagen T-Cross evidence identifies Highline 250 TSI and the case-bound MY26 owner manual explicitly maps the 1.4 Total Flex 110 kW / 250 Nm TSI engine family to the AQ250 six-speed automatic transmission. Adding only that missing transmission field reduced the then-current replay to 19 `CREATED`, 23 `MATCHED`, 18 `REVIEW`, and zero failures. The separate T-Cross one-sided-model-year case remained `REVIEW`; no model year was inferred onto the yearless observation.

PR #105 later corrected precedence so an independent explicit structural contradiction can win over lexical partial-label overlap while preserving `REVIEW` when no such contradiction exists. On the same bounded enriched V3 corpus, that resolver correction changes the current replay to 21 `CREATED`, 26 `MATCHED`, 13 `REVIEW`, and zero failures. This is a resolver-correctness effect, not additional evidence enrichment. The 13 remaining cases are the review set targeted by the fail-closed disposition work in PR #106.

The companion source-backed identity quality corpus remains at auto-match precision 1.0 and recall 1.0 with zero false merges and zero ambiguous overcommit. Resolver-policy changes remain zero for the enrichment cycles themselves. Enrichment sources remain bound to each curated case and overlapping field mutations fail closed.
