...
Status: `OPEN / OFFICIAL ALTERNATIVE PATHS SELECTED + FROZEN ACQUISITION + DIRECT HTTP TRANSPORT + LIVE SAMPLES + CROSS-FAMILY EXTRACTION LOCALLY_VERIFIED`
...
The repository therefore has local deterministic evidence for retained-snapshot lineage, direct HTTP transport semantics, extraction reuse across two frozen web source families, bounded real-network compatibility observations, a bounded normal-browser refusal characterization for Autoevolution, and current official alternative-source selection evidence. It still does **not** establish heterogeneous-web or production precision/recall, production source distribution, source-discovery completeness, recurring robots/rate/politeness behavior, comprehensive SSRF/DNS-rebinding defense, automated current Inmetro CSV acquisition, PDF extraction correctness for PBEV, or a general MPGe semantic mapping.

The generic debt **"find any compliant alternative live source" is closed**. Remaining acquisition/generalization debt is now concrete:

1. implement and independently benchmark a bounded **EEA source-family V1** before treating EEA rows as production facts;
2. implement bounded **official source discovery V1** using NHTSA vPIC and the already-verified FuelEconomy.gov menu flow, preserving discovery as candidate generation rather than identity proof;
3. characterize an exact current **Inmetro PBEV machine-readable or deliberate document path** only when Brazilian coverage is the active need; do not weaken global validation just to make the PDF/401 surfaces pass;
4. define recurring-operation robots/rate/politeness and host pacing before scheduling broad live acquisition;
5. strengthen network target binding before accepting arbitrary untrusted locators;
6. broaden production source distribution/corpus coverage only through independently inspected source-family evidence and measured failure modes.
