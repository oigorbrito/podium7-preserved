...
The generic debts “find any compliant alternative live source”, “implement EEA source-family V1”, and “implement bounded official source discovery V1” are closed.

Remaining acquisition/generalization debt is concrete:

1. **Recurring-operation source policy.** Define robots/rate/politeness interpretation and host pacing before scheduling broad live acquisition. Existing source-specific documented rate controls still apply immediately.
2. **Network target binding.** Strengthen protection against SSRF/DNS rebinding before accepting arbitrary untrusted locators. The current direct-HTTP boundary remains intentionally narrower than that future capability.
3. **Inmetro PBEV path, when needed.** Characterize an exact current unauthenticated machine-readable resource or a deliberate source-specific PDF/document acquisition-and-extraction path only when Brazilian coverage is an active product need. Do not weaken the default global HTTP media-type or validation policy merely to make the current PDF/401 surfaces pass.
4. **Production source distribution and corpus breadth.** Broaden coverage only through independently inspected source-family evidence and measured failure modes. Current bounded results do not establish heterogeneous-web or production precision/recall, discovery completeness, or production-wide European/U.S./Brazilian coverage.
5. **EEA semantic expansion.** V1 does not establish diesel/diesel-electric mappings, production-wide European coverage, or canonical retail identity from EEA type/variant/version evidence. Expand only when those semantics become decision-critical and independently benchmarked.
